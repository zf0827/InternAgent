"""
GroundingAgentV2 - Idea-Report Level Grounding
...
class GroundingAgentV2(BaseAgent):
    """
    ...
    async def _do_extract(self, report_type: str, report: Dict, 
                          idea_part: str, part_name: str) -> Tuple[str, float]:
        """
...
2. Analyze the report content thoroughly
3. Extract the most relevant content that relates to or supports the idea part
4. Write a concise, informative summary (2-5 sentences) that captures the key connections
5. Score the relevance/support strength on a 0-10 scale:
...
Be precise, objective, and focus on factual connections between the report and the idea part."""
    
    def _build_extract_prompt(self, report_type: str, report_content: str,
                             idea_part: str, part_name: str, report: Dict) -> str:
        """Build extraction prompt based on report type"""
        if report_type == "paper_report":
            return self._build_paper_extract_prompt(report_content, idea_part, part_name, report)
        elif report_type == "web_report":
            return self._build_web_extract_prompt(report_content, idea_part, part_name, report)
        elif report_type == "code_report":
            return self._build_code_extract_prompt(report_content, idea_part, part_name, report)
        else:
            raise ValueError(f"Unknown report type: {report_type}")
    
    def _build_paper_extract_prompt(self, report_content: str, idea_part: str,
                                   part_name: str, report: Dict) -> str:
        """Build extraction prompt for paper reports"""
        metadata = report.get("paper_metadata", {})
        title = metadata.get("title", "Unknown Paper")
        
        return f"""## Task: Extract Relevant Content from Related Paper
...
## Output:
Provide a summary and score that reflects how this related paper's content connects to our research idea part."""
    ...
    def _build_web_extract_prompt(self, report_content: str, idea_part: str,
        ...
        return f"""## Task: Extract Relevant Views and Perspectives from Web Content
...
## Output:
Provide a summary capturing the key viewpoints and perspectives, along with a relevance score."""
    ...
    def _build_code_extract_prompt(self, report_content: str, idea_part: str,
        ...
        return f"""## Task: Extract Implementation and Experimental Contributions from Code Repository
...
## Output:
Provide a summary of the repository's implementation and experimental contributions, along with a relevance score."""
